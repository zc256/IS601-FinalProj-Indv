...
def index():
    ...
    cursor.execute('SELECT * FROM mlb_players_1')
...
def record_view(player_id):
    cursor = mysql.get_db().cursor()
    cursor.execute('SELECT * FROM mlb_players_1 WHERE id=%s', player_id)
    result = cursor.fetchall()
...
def form_update_post(player_id):
    cursor = mysql.get_db().cursor()
    inputData = (request.form.get('Player'), request.form.get('Team'), request.form.get('Position'),
                 request.form.get('Height_inches'), request.form.get('Weight_lbs'),
                 request.form.get('Age'), player_id)
    sql_update_query = """UPDATE mlb_players_1 t SET t.Player = %s, t.Team = %s, t.Position = %s, t.Height_inches = 
...
def form_insert_post():
    ...
    sql_insert_query = """INSERT INTO mlb_players_1 (Player,Team,Position,Height_inches,
    Weight_lbs,Age) VALUES (%s, %s,%s, %s,%s, %s) """
...
def form_delete_post(player_id):
    cursor = mysql.get_db().cursor()
    sql_delete_query = """DELETE FROM mlb_players_1 WHERE id = %s """
...
def api_edit(player_id) -> str:
    ...
    inputData = (content['Player'], content['Team'], content['Position'],
...
@app.route('/api/v1/players', methods=['POST'], strict_slashes=False)
def api_add() -> str:
    content = request.json
    cursor = mysql.get_db().cursor()
    inputData = (content['Player'], content['Team'], content['Position'],
